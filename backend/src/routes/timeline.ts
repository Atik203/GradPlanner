...
// Helper to parse intake month & year
const parseIntake = (intakeStr: string) => {
  const parts = intakeStr.trim().split(/\s+/);
  if (parts.length !== 2) return { month: 8, year: 2028 }; // Default Sep 2028
  const monthStr = parts[0].toLowerCase();
  const year = parseInt(parts[1], 10) || 2028;
  
  let month = 8; // Default September (0-indexed is 8)
...
// Helper to calculate date by month offsets
...
// GET /api/v1/timeline/planner
router.get("/planner", async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    const queryIntake = req.query.intake as string | undefined;

    // 1. Fetch user data
    const [profile, documents, professors, applications] = await Promise.all([
      prisma.userProfile.findUnique({ where: { userId } }),
      prisma.document.findMany({ where: { userId } }),
      prisma.professor.findMany({ where: { userId, deletedAt: null } }),
      prisma.application.findMany({ where: { userId, deletedAt: null } })
    ]);

    // 2. Resolve target intake
    ...
    // 3. Define timeline milestones using strict server-side rules
    ...
    // Graduation collection is fixed relative to Nov 2027 (standard UIU/BD graduation)
    // If winter intake, let the collection start earlier
    const gradStart = targetMonth === 0 ? getDateByOffset(targetYear, targetMonth, 17) : getDateByOffset(targetYear, targetMonth, 13);
    ...
    const programEnd = getDateByOffset(targetYear, targetMonth, -1, 30); // 1 month in program

    // 4. Run status evaluation checks
    const hasIelts = documents.some(d => d.type === "IELTS" && d.status === "OBTAINED") || 
                     documents.some(d => d.type === "TOEFL" && d.status === "OBTAINED") || 
                     (profile?.ieltsScore && profile.ieltsScore >= 5.0);
    
    const contactedCount = professors.filter(p => p.status !== "NOT_CONTACTED").length;
    
    const hasSop = documents.some(d => d.type === "SOP" && d.status === "OBTAINED");
    const hasCv = documents.some(d => d.type === "CV" && d.status === "OBTAINED");
    const hasLor = documents.some(d => d.type === "LOR" && d.status === "OBTAINED");

    const hasTranscript = documents.some(d => d.type === "TRANSCRIPT" && d.status === "OBTAINED") || 
                          documents.some(d => d.type === "DEGREE_CERTIFICATE" && d.status === "OBTAINED");
    
    const hasPcc = documents.some(d => d.type === "POLICE_CLEARANCE" && d.status === "OBTAINED");
    
    const hasSubmittedApps = applications.some(a => ["SUBMITTED", "UNDER_REVIEW", "OFFER_RECEIVED", "ACCEPTED"].includes(a.status));
    
    const hasVisaOrFinancials = documents.some(d => d.type === "BANK_STATEMENT" && d.status === "OBTAINED") ||
                                 applications.some(a => a.status === "ACCEPTED" || a.offerReceived);
    ...
    const milestones = [
      {
        ...
        description: "Complete preparation and exams. IELTS seats in Dhaka fill up 6–8 weeks ahead. A minimum band of 6.5 is recommended for STEM admissions.",
        icon: "ielts"
      },
      {
        id: "outreach",
      ...
      },
      {
        ...
        status: evaluateStatus(!!(hasSop && hasCv && hasLor), !!(hasSop || hasCv || hasLor), draftingStart, draftingEnd),
        description: "Draft your Statement of Purpose (SOP), Curriculum Vitae (CV), and request reference letters (LORs) from your university registrar / professors.",
        icon: "drafting"
      },
      {
        id: "documents",
        ...
        description: "Collect official transcripts and degree certificates post-graduation. Processing times at local universities (like UIU) can take 3-7 days.",
        icon: "documents"
      },
      {
        id: "police",
        ...
        description: "Apply for a Police Clearance Certificate (PCC) online (pcc.police.gov.bd). Processing takes 2-6 weeks via Ramna HQ in Dhaka.",
        icon: "police"
      },
      {
        id: "applications",
        ...
        description: "Submit all online applications before the priority deadline. Keep close track of hard deadlines like Sweden (Jan 15) or USA winter cuts.",
        icon: "applications"
      },
      {
        id: "visa",
        ...
        description: "Set up Blocked Account (Germany Fintiba/Coracle) or wire GIC (Canada, takes 5-10 days). Submit visa file to VFS / embassy in Dhaka.",
        icon: "visa"
      },
      {
        id: "program",
        ...
        status: evaluateStatus(hasAcceptedOffer && serverTime >= programStart, false, programStart, programEnd),
    ...
    console.error("GET /timeline/planner error:", error);
    res.status(500).json({ error: "Failed to generate timeline plan" });
  }
});
